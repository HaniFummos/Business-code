...
bertscorer = BERTScorer(model_type="bert-base-multilingual-cased", lang="nl")
...
def flesch_douma(text):
    ...
    def count_syllables(w):
        ...
        while i < len(w):
            if w[i] in vowels:
                count += 1
                while i+1 < len(w) and w[i+1] in vowels: i += 1
            i += 1
        return max(1, count)
    
    avg_words = len(words)/len(sentences)
    avg_syllables = sum(count_syllables(w) for w in words)/len(words)
    return max(0, min(100, 206.84 - (0.93*avg_words) - (65.6*avg_syllables)))
...
def process(pdf_path, temperature, idx, total):
    ...
    read = flesch_douma(summary)
    bert = bertscorer.score([summary], [original])[2].item()
    ...
    print(f"  -> [{idx}/{total}] Temp {temperature}: Read={read:.2f} | BERT={bert:.3f} | Chars={char_count}")
    return {'temperature': temperature, 'read': read, 'bert': bert, 'chars': char_count}
...
if results:
    ...
    for temp in TEMPERATURES:
        ...
        if r:
            avg_read = sum(x['read'] for x in r)/len(r)
            avg_bert = sum(x['bert'] for x in r)/len(r)
            avg_chars = sum(x['chars'] for x in r)/len(r)
            print(f"Temp = {temp}: Read={avg_read:.2f} | BERT={avg_bert:.3f} | Chars={avg_chars:.0f}")
    print("="*60)